...
const toTitleCase = (arr) => {
  return arr.map(n => n.charAt(0).toUpperCase() + n.slice(1));

  //uppercases the first letter, and then slices the string and returns it starting from the second character
...
let biggerThanLuke = (arr) => {
  const humans = arr.filter(person => person.mass > 77);
  return humans.map(person => person.name).join(" - ");
};
...
//   var name_order = a.name.localeCompare(b.name);
//   var price_order = a.price.localeCompare(b.price);
//   return name_order || price_order;
// })
