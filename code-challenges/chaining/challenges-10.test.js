...
const count = (target, input) => {
  // let newInput = input.concat(...newInput);
  let newInput = [];
  for (let i = 0; i < input.length; i++) {
    newInput = newInput.concat(input[i]);
  }
  return newInput.reduce((accumulator, value) => {
    if (value === target) {
...
const totalSum = (input) => {
  ...
  let result = newInput.reduce((accumulator, value) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  let result = [];
  for (let i = 0; i < input.length; i++) {
    let temp = input[i].filter(n => (typeof n === "number" && n % 5 === 0));
    result.push(temp.map(n => 2 ** n));
  }
  return result;

  // let check = isNaN(n);
  // if (check === false && n % 5 === 0) {
  //   return check.map(n => n ** 2);
  // }
  // )

  // const numbers = input.map(arr => arr.filter(val=> typeof(val)=== 'number && !(val % 5)));
  // return numbers.map(arr => arr.mpa(val => Math.pow(2, val)))
};
...
let findMaleAndFemale = (data) => {
  ...
  return result.length > 0 ? result.slice(0, result.length - 5) : result;
};
...
let findShortest = (data) => {
  return data.reduce((accumulator, person, index) => {
    if (index === 0) {
      return person;
    }
    if (parseInt(person.height) < parseInt(accumulator.height)) {
      return person;
    }
    return accumulator;
  }, {}).name;
